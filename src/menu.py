...
class FirstForm(QMainWindow):
    ...
    def initUI(self):
        ...
        self.pioneer_csv_btn.setToolTip(
            "Нажмите, если у вас готовый csv файл с пачкой url"
        )
        ...
        self.pioneer_test_csv_btn.setToolTip(
            "Нажмите, если это тестовый прогон")
...
class SecondForm(QWidget):
    ...
    def csv_saver_func(self):
        self.path = QFileDialog.getSaveFileName(self, "Open File", "",
                                                "(*.csv)")
...
class Csv_test_progon(QWidget):
    ...
    def ecran(self, tablica=False, sec_tablica=False):
            ...
            if all(len(z) == 2 for z in self.inside_csv):
                ...
                for i in range(len(self.inside_csv)):
                    self.main_table.setItem(
                        i, 0, QTableWidgetItem(self.inside_csv[i][0])
                    )
                    self.main_table.setItem(
                        i, 2, QTableWidgetItem(self.inside_csv[i][1])
                    )
                ...
                if sec_tablica:
                    ...
                    for i in range(len(self.vtoroy_stolbec)):
                        self.main_table.setItem(
                            i, 1, QTableWidgetItem(self.vtoroy_stolbec[i])
                        )
                        if (
                            self.main_table.item(i, 1).text().strip()
                            == self.main_table.item(i, 2).text().strip()
                        ):
                            self.main_table.item(i, 1).setBackground(
                                QColor(0, 250, 0))
                        else:
                            self.main_table.item(i, 1).setBackground(
                                QColor(250, 0, 0))
                        if self.main_table.item(i, 1).text().strip() == "error":
                            self.main_table.item(i, 0).setBackground(
                                QColor(250, 0, 0))
                            self.main_table.item(i, 1).setBackground(
                                QColor(250, 0, 0))
                            self.main_table.item(i, 2).setBackground(
                                QColor(250, 0, 0))
        ...
        self.main_table.setHorizontalHeaderLabels(
            ["Url", "Результат", "Правильный"])
        self.main_table.setHidden(True)

    def dialog(self):
        self.file, self.checker = QFileDialog.getOpenFileName(
            None, "QFileDialog.getOpenFileName()", "", "All Files (*.csv)"
        )
        if self.checker:
            self.inside_csv = []
            ...
            if all(len(z) == 1 for z in self.csv_in):
                ...
                if sec_tablica:
                    ...
                    for i in range(len(self.second_table)):
                        self.main_table.setItem(
                            i, 1, QTableWidgetItem(self.second_table[i])
                        )
                        if self.main_table.item(i, 1).text().strip() == "error":
                            self.main_table.item(i, 1).setBackground(
                                QColor(255, 0, 0))
